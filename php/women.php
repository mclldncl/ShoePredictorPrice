...
if (isset($_SESSION['user_id'])) {
  ...
  $res = $conn->prepare("SELECT SUM(quantity) AS total FROM cart_items WHERE user_id = ?");
...
// Count total products in women's category
$count_result = $conn->query("SELECT COUNT(*) AS total FROM products WHERE category = 'women'");
$total = $count_result->fetch_assoc()['total'];
$total_pages = ceil($total / $limit);

// Fetch products for this page
$stmt = $conn->prepare("SELECT * FROM products WHERE category = 'women' LIMIT ?, ?");
...
<head>
  <meta charset="UTF-8" />
  <title>Women's Shoes | ShoelyYours</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap" rel="stylesheet">
  <style>
    body { font-family: 'Poppins', sans-serif; background-color: #f9f9f9; }
    .navbar { background-color: #fff; border-bottom: 1px solid #ddd; padding: 1rem 0.5rem; }
    ...
    .hero {
      height: 80vh;
      background: url('images/women-banner.jpg') center center/cover no-repeat;
      position: relative;
      display: flex;
      align-items: center;
    ...
    .desc h5 { font-size: 1.1rem; margin-top: 10px; }
  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse justify-content-center" id="mainNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item"><a class="nav-link fw-bold text-primary" href="women.php">Women</a></li>
...
<section class="hero mt-5">
  <div class="content">
    ...
    <p class="lead">Elegance in Every Step</p>
  </div>
</section>
    ...
    <?php while ($row = $products->fetch_assoc()): ?>
      <div class="col">
        <div class="product-entry">
          <img src="images/<?= htmlspecialchars($row['image']) ?>" class="product-img" alt="<?= htmlspecialchars($row['name']) ?>">
